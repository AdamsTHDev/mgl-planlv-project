...
public class MTIKBankPlanLvGenImpl extends AbstractPlanLevelGenerator {
...
//		[0] is campaignCode, [1] is approve yearMonth
		List<PlanLvValue> planLvList = service.findByNamedQuery("execPlanLvValueForMTIKBankMTD", campaignCode, DateUtil.convDateToString("yyyyMM", processDate));
	...
	public PlanLevelObj getYTDData(String campaignCode, Date processDate) throws Exception {
		PlanLvValueService service = (PlanLvValueService) ApplicationContextUtil.getApplicationContext().getBean("planLvValueService");
//		[0] is campaignCode, [1] is approve yearMonth
		List<PlanLvValue> planLvList = service.findByNamedQuery("execPlanLvValueForMTIKBankYTD", campaignCode, DateUtil.convDateToString("yyyyMM", processDate));
		PlanLevelObj result = new PlanLevelObj();
		result.setCampaignCode(campaignCode);
...
//		Spouse are Cols F to I
	...
	private int getPlanColumnIdx(Sheet sheet, String productType, String planType) {
		...
		if(productType.toUpperCase().equals("INDIVIDUAL")) {
			section = "INDIVIDUAL";
		} else if(productType.toUpperCase().equals("SPOUSE")) {
			section = "SPOUSE";
		} else if(productType.toUpperCase().equals("KIDS")) {
			section = "Main + Child";
		} else if(productType.toUpperCase().equals("FAMILY")) {
			section = "Main + Spouse + Child";
		}
		...
		for(int n = 0; n < sheet.getRow(planRowIdx - 1).getLastCellNum(); n++) {
			...
			if(cell.getStringCellValue().toUpperCase().contains(section.toUpperCase())) {
