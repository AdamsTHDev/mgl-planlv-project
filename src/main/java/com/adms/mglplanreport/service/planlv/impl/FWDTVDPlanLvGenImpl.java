...
public class FWDTVDPlanLvGenImpl extends AbstractPlanLevelGenerator {
...
//		[0] is campaignCode, [1] is approve yearMonth
		List<PlanLvValue> planLvList = service.findByNamedQuery("execPlanLvValueForFWDTVDMTD", campaignCode, DateUtil.convDateToString("yyyyMM", processDate));
	...
	public PlanLevelObj getYTDData(String campaignCode, Date processDate) throws Exception {
		PlanLvValueService service = (PlanLvValueService) ApplicationContextUtil.getApplicationContext().getBean("planLvValueService");
//		[0] is campaignCode, [1] is approve yearMonth
		List<PlanLvValue> planLvList = service.findByNamedQuery("execPlanLvValueForFWDTVDYTD", campaignCode, DateUtil.convDateToString("yyyyMM", processDate));
		PlanLevelObj result = new PlanLevelObj();
		result.setCampaignCode(campaignCode);
